...
def view_compound_receipt(body: CompoundCreate):
    ...
    compound_no = compound.compoundnum
    compound_plate = compound.plate
    compound_date = compound.date
    compound_time = compound.time
    compound_offense = compound.offense
    compound_amount = compound.amount
    
    # --- Generate PDF from EXTERNAL FILE ---
    pdf_url = generate_single_compound_pdf(compound)
...
def generate_multi_compound_receipt(payload: dict):
    ...
    pdf_buffer = generate_multi_compound_pdf(compounds, total_amount)
    ...
    # --- 3) Build HTML (ADD PDF BUTTON HERE) ---
    html = f"""
   <html>
   <head>
